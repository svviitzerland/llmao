...
impl fmt::Display for LlmaoError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            LlmaoError::Auth(msg) => write!(f, "Authentication failed: {}", msg),
            LlmaoError::Timeout(msg) => write!(f, "Request timeout: {}", msg),
            LlmaoError::Internal(msg) => write!(f, "Internal error: {}", msg),
        }
    }
}
...
impl From<LlmaoError> for PyErr {
    fn from(err: LlmaoError) -> PyErr {
        match err {
            LlmaoError::Config(msg) => PyValueError::new_err(msg),
            LlmaoError::ProviderNotFound(msg) => PyValueError::new_err(msg),
            LlmaoError::ModelNotSupported { provider, model } => {
            ...
            LlmaoError::Request(msg) => PyConnectionError::new_err(msg),
            LlmaoError::Response(msg) => PyRuntimeError::new_err(msg),
            LlmaoError::Stream(msg) => PyRuntimeError::new_err(msg),
            LlmaoError::Auth(msg) => PyRuntimeError::new_err(format!("Auth error: {}", msg)),
            LlmaoError::Timeout(msg) => PyConnectionError::new_err(msg),
            LlmaoError::Internal(msg) => PyRuntimeError::new_err(msg),
        }
    }
}
